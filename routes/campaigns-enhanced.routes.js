...
// CYBEV Enhanced Campaign API - FULLY FIXED
// VERSION: 3.0.0 - Fixed route order + all features
// ============================================
...
if (!Campaign) {
  const campaignSchema = new mongoose.Schema({
    ...
    audience: { type: { type: String, enum: ['all', 'segment', 'tags', 'list'], default: 'all' }, contactList: mongoose.Schema.Types.ObjectId, segment: mongoose.Schema.Types.ObjectId, tags: [String], excludeTags: [String] },
...
if (!EmailContact) {
  const contactSchema = new mongoose.Schema({
    ...
    tags: [String], customFields: mongoose.Schema.Types.Mixed,
    subscribed: { type: Boolean, default: true }, subscribedAt: { type: Date, default: Date.now }, unsubscribedAt: Date,
    source: { type: String, default: 'manual' }, engagementScore: { type: Number, default: 50 }
  }, { timestamps: true });
...
if (!ContactList) {
  const listSchema = new mongoose.Schema({
    ...
    subscriberCount: { type: Number, default: 0 }, activeCount: { type: Number, default: 0 },
    defaultTags: [String], customFields: [{ name: String, type: String, required: Boolean }]
  }, { timestamps: true });
...
if (!EmailAddress) {
  const addressSchema = new mongoose.Schema({
    user: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
    email: { type: String, required: true }, displayName: String,
...
router.post('/lists', auth, async (req, res) => {
  try {
    ...
    const list = await ContactList.create({ user: userId, name: name || 'New List', description, defaultTags, customFields });
    res.json({ ok: true, list });
  } catch (err) { res.status(500).json({ error: 'Failed to create list' }); }
});

router.post('/lists/:listId/import', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const { contacts } = req.body;
    const list = await ContactList.findOne({ _id: req.params.listId, user: userId });
    if (!list) return res.status(404).json({ error: 'List not found' });
    let imported = 0, skipped = 0;
    for (const contact of contacts || []) {
      if (!contact.email) { skipped++; continue; }
      try {
        await EmailContact.findOneAndUpdate({ user: userId, email: contact.email.toLowerCase() }, { $set: { name: contact.name, firstName: contact.firstName, lastName: contact.lastName, phone: contact.phone, company: contact.company, source: 'import' }, $addToSet: { tags: { $each: list.defaultTags || [] } } }, { upsert: true, new: true });
        imported++;
      } catch (e) { skipped++; }
    }
    ...
    await list.save();
    res.json({ ok: true, imported, skipped, total: list.subscriberCount });
...
router.get('/contacts', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const { page = 1, limit = 50, search, tag, subscribed } = req.query;
...
router.post('/contacts', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const { email, name, firstName, lastName, phone, company, tags } = req.body;
    if (!email) return res.status(400).json({ error: 'Email is required' });
    const contact = await EmailContact.findOneAndUpdate({ user: userId, email: email.toLowerCase() }, { $set: { name: name || `${firstName || ''} ${lastName || ''}`.trim(), firstName, lastName, phone, company, source: 'manual' }, $addToSet: tags?.length ? { tags: { $each: tags } } : {} }, { upsert: true, new: true });
    res.json({ ok: true, contact });
  } catch (err) { if (err.code === 11000) return res.status(400).json({ error: 'Contact already exists' }); res.status(500).json({ error: 'Failed to create contact' }); }
});
...
router.post('/contacts/import', auth, async (req, res) => {
  try {
    ...
    for (const contact of contacts) {
      if (!contact.email) { skipped++; continue; }
      try {
        await EmailContact.findOneAndUpdate({ user: userId, email: contact.email.toLowerCase() }, { $set: { name: contact.name || `${contact.firstName || ''} ${contact.lastName || ''}`.trim(), firstName: contact.firstName, lastName: contact.lastName, phone: contact.phone, company: contact.company, source: 'import' }, $addToSet: tags.length ? { tags: { $each: tags } } : {} }, { upsert: true });
        imported++;
      } catch (e) { skipped++; errors.push({ email: contact.email, error: e.message }); }
    }
...
router.post('/templates', auth, async (req, res) => {
  try {
    ...
    const template = await EmailTemplate.create({ user: userId, name: name || 'Untitled Template', description, category: category || 'general', subject, previewText, content: { html: content?.html || '', text: content?.text || '', blocks: blocks || content?.blocks || [] }, thumbnail, type: 'user' });
...
router.get('/tags', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const tags = await EmailContact.aggregate([{ $match: { user: new mongoose.Types.ObjectId(userId) } }, { $unwind: '$tags' }, { $group: { _id: '$tags', count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 100 }]);
...
router.get('/addresses', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const addresses = await EmailAddress.find({ user: userId, isActive: true }).lean();
    if (!addresses.length) return res.json({ addresses: [{ _id: 'default', email: process.env.SES_FROM_EMAIL || 'noreply@cybev.io', displayName: 'CYBEV', isVerified: true, isActive: true }] });
    res.json({ addresses });
  } catch (err) { res.status(500).json({ error: 'Failed to fetch addresses' }); }
});

// ==========================================
// CAMPAIGN CRUD (/:id routes LAST)
// ==========================================
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const [campaigns, total] = await Promise.all([Campaign.find(query).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(parseInt(limit)).lean(), Campaign.countDocuments(query)]);
...
router.post('/', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const { name, type = 'email', subject, previewText, content, sender, audience, schedule, tracking } = req.body;
    if (!name) return res.status(400).json({ error: 'Campaign name is required' });
    let senderData = { email: sender?.email || process.env.SES_FROM_EMAIL || 'noreply@cybev.io', name: sender?.name || 'CYBEV', replyTo: sender?.replyTo };
    let recipientCount = 0;
    if (audience?.type === 'all' || !audience?.type) {
      recipientCount = await EmailContact.countDocuments({ user: userId, subscribed: true });
    } else if (audience?.tags?.length > 0) {
      recipientCount = await EmailContact.countDocuments({ user: userId, subscribed: true, tags: { $in: audience.tags } });
    }
    const campaign = await Campaign.create({
      user: userId, name, type, subject: subject || '', previewText: previewText || '',
      content: { html: content?.html || '', text: content?.text || '', json: content?.json, blocks: content?.blocks || [] },
      sender: senderData,
      audience: { type: audience?.type || 'all', contactList: audience?.contactList, tags: audience?.tags || [], excludeTags: audience?.excludeTags || [] },
      schedule: { type: schedule?.type || 'immediate', scheduledAt: schedule?.scheduledAt, timezone: schedule?.timezone || 'UTC' },
      tracking: { openTracking: tracking?.openTracking !== false, clickTracking: tracking?.clickTracking !== false, googleAnalytics: tracking?.googleAnalytics },
      stats: { recipientCount }
    });
    res.json({ ok: true, campaign });
  } catch (err) { console.error('Create campaign error:', err); res.status(500).json({ error: 'Failed to create campaign' }); }
});
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    ['name', 'subject', 'previewText', 'content', 'sender', 'audience', 'schedule', 'tracking'].forEach(field => { if (updates[field] !== undefined) campaign[field] = updates[field]; });
    await campaign.save();
    res.json({ ok: true, campaign });
...
router.post('/:id/duplicate', auth, async (req, res) => {
  try {
    ...
    const copy = { ...original, _id: undefined, name: `${original.name} (Copy)`, status: 'draft', createdAt: undefined, updatedAt: undefined, sentAt: undefined, stats: { recipientCount: original.stats?.recipientCount || 0 }, sending: undefined };
...
router.post('/:id/test', auth, async (req, res) => {
  try {
    ...
    if (!testEmails.length || !testEmails[0]) return res.status(400).json({ error: 'Test email address required' });
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ error: 'Invalid campaign ID' });
    ...
    for (const email of testEmails) {
      try {
        const result = await sesService.sendEmail({ to: email, subject: `[TEST] ${campaign.subject || 'Test Campaign'}`, html: campaign.content?.html || '<p>Test email content</p>', text: campaign.content?.text, from: `${campaign.sender?.name || 'CYBEV'} <${campaign.sender?.email || process.env.SES_FROM_EMAIL}>`, replyTo: campaign.sender?.replyTo });
        results.push({ email, success: true, messageId: result.messageId });
      } catch (err) { results.push({ email, success: false, error: err.message }); }
    }
...
router.post('/:id/send', auth, async (req, res) => {
  try {
    ...
    if (campaign.audience?.type === 'tags' && campaign.audience.tags?.length) query.tags = { $in: campaign.audience.tags };
    ...
    await campaign.save();
    const recipients = contacts.map(c => ({ campaign: campaign._id, contact: c._id, email: c.email, name: c.name || `${c.firstName || ''} ${c.lastName || ''}`.trim(), status: 'queued' }));
...
async function processCampaignSending(campaignId, userId) {
  try {
    ...
    while (currentBatch < totalBatches) {
      ...
      const batch = recipients.map(r => ({ email: r.email, name: r.name, data: { firstName: r.name?.split(' ')[0] || 'there' } }));
      try {
        const result = await sesService.sendBulkEmails({ recipients: batch, subject: campaign.subject, html: campaign.content.html || '<p>Email content</p>', text: campaign.content.text, from: `${campaign.sender?.name || 'CYBEV'} <${campaign.sender?.email || process.env.SES_FROM_EMAIL}>`, campaignId: campaignId.toString() });
        for (const res of result.results || []) {
          const status = res.success ? 'sent' : 'failed';
          await CampaignRecipient.findOneAndUpdate({ campaign: campaignId, email: res.email }, { status, sesMessageId: res.messageId, sentAt: res.success ? new Date() : undefined, error: res.error ? { message: res.error } : undefined });
          if (res.success) sent++; else failed++;
        }
      } catch (e) { console.error('Batch send error:', e); }
      const progress = Math.round((currentBatch / totalBatches) * 100);
      await Campaign.findByIdAndUpdate(campaignId, { 'sending.progress': progress, 'sending.currentBatch': currentBatch, 'stats.sent': sent, 'stats.bounced': failed });
      await new Promise(resolve => setTimeout(resolve, 200));
    }
    await Campaign.findByIdAndUpdate(campaignId, { status: 'sent', sentAt: new Date(), 'sending.completedAt': new Date(), 'sending.progress': 100, 'stats.sent': sent, 'stats.delivered': sent });
    console.log(`📧 Campaign ${campaignId} completed: ${sent} sent, ${failed} failed`);
  } catch (err) { console.error(`Campaign ${campaignId} error:`, err); await Campaign.findByIdAndUpdate(campaignId, { status: 'paused', 'sending.lastError': err.message }); }
}

router.post('/:id/pause', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const campaign = await Campaign.findOneAndUpdate({ _id: req.params.id, user: userId, status: 'sending' }, { status: 'paused', 'sending.pausedAt': new Date() }, { new: true });
...
router.post('/:id/resume', auth, async (req, res) => {
  try {
    const userId = req.user.userId || req.user.id;
    const campaign = await Campaign.findOneAndUpdate({ _id: req.params.id, user: userId, status: 'paused' }, { status: 'sending' }, { new: true });
    if (!campaign) return res.status(404).json({ error: 'Campaign not found' });
    processCampaignSending(campaign._id, userId).catch(console.error);
    res.json({ ok: true, campaign });
...
router.post('/:id/schedule', auth, async (req, res) => {
  try {
    ...
    const campaign = await Campaign.findOneAndUpdate({ _id: req.params.id, user: userId, status: 'draft' }, { status: 'scheduled', 'schedule.type': 'scheduled', 'schedule.scheduledAt': new Date(scheduledAt), 'schedule.timezone': timezone || 'UTC' }, { new: true });
...
router.get('/:id/recipients', auth, async (req, res) => {
  try {
    ...
    const [recipients, total] = await Promise.all([CampaignRecipient.find(query).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(parseInt(limit)), CampaignRecipient.countDocuments(query)]);
...
router.get('/unsubscribe', async (req, res) => {
  ...
  try {
    ...
    if (campaign && mongoose.Types.ObjectId.isValid(campaign)) campaignDoc = await Campaign.findById(campaign);
    if (campaignDoc?.user) {
      await Unsubscribe.findOneAndUpdate({ email: email.toLowerCase(), user: campaignDoc.user }, { source: 'link', campaign: campaignDoc._id }, { upsert: true });
      await EmailContact.findOneAndUpdate({ email: email.toLowerCase(), user: campaignDoc.user }, { subscribed: false, unsubscribedAt: new Date() });
    }
